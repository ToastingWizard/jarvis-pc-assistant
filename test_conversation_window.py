"""
Regression tests for the bug where JARVIS would react to background
...
These tests pin down the fixed behavior directly on JarvisEngine, which
holds the actual logic (JarvisUI.voice_loop just calls into it).
"""
...
def test_singing_style_phrases_are_not_addressed_to_jarvis(engine):
    """Common song-lyric phrasing should not, on its own, count as
    'was_addressed_to_jarvis' just because it contains a command-shaped
    ...
    for lyric in lyrics:
        assert engine.was_addressed_to_jarvis(lyric) is False


def test_addressed_requires_wake_word_or_name(engine):
    assert engine.was_addressed_to_jarvis("hey jarvis open chrome") is True
    assert engine.was_addressed_to_jarvis("jarvis, what time is it") is True
    assert engine.was_addressed_to_jarvis("what a lovely day") is False
...
def test_command_shaped_lyric_is_actionable_but_not_addressed(engine):
    ...
    hands-free follow-up commands convenient) — but JarvisUI.voice_loop
    ...
    assert engine.was_addressed_to_jarvis(lyric) is False  # ...but wasn't addressed
